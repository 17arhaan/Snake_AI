...
class SnakeQAgent:
    def __init__(self, episodes):
        ...
        self.exp_decay = 0.9992  # Exploration decay
        self.min_exp = 0.001  # Minimum exploration rate
        self.episodes = episodes  # Number of episodes
        self.q_table = np.zeros((2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 4))  # Adjust state space size
        self.env = LearnSnake()  # Game environment
        self.scores = []
        self.survived = []
        ...
        for ep in range(1, self.episodes + 1):
            ...
            while not done:
                ...
                if length != self.env.snake_length:
                    length = self.env.snake_length
                    self.scores.append(length)

            self.survived.append(steps_no_food)

def main():
    episodes = int(input("Number of episodes or trials to Train: "))
