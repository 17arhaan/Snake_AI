...
class Color:
    def __init__(self):
        self.white = (255, 255, 255)
        self.off_white = (240, 240, 240)  # Off-white color
        self.black = (0, 0, 0)
        self.neon_red = (255, 0, 0)
        self.blue = (0, 0, 255)
...
class SnakeGame:
    def __init__(self):
        ...
        # Load images
        self.head_image = pygame.image.load("snake_head.png").convert_alpha()
        self.food_image = pygame.image.load("food.png").convert_alpha()
        self.head_image = pygame.transform.scale(self.head_image, (self.sn_size, self.sn_size))
        self.food_image = pygame.transform.scale(self.food_image, (self.fd_size, self.fd_size))
    ...
    def draw_snake(self):
        for i in range(len(self.sn_coords) - 1, -1, -1):
            ...
            x, y = self.idx_to_coord(row, col)
            if i == len(self.sn_coords) - 1:
                self.screen.blit(self.head_image, (x, y))  # Head
            else:
                pygame.draw.rect(self.screen, self.color.off_white, [x, y, self.sn_size, self.sn_size])  # Body
    ...
    def step(self, action="None"):
        ...
        fd_x, fd_y = self.idx_to_coord(self.fd_r, self.fd_c)
        self.screen.blit(self.food_image, [fd_x, fd_y])  # Food
